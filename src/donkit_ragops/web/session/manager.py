...
class SessionManager:
    ...
    async def create_enterprise_session(
        ...
        # Get API token: from parameter or keyring
        token = api_token
        ...
        if not token:
            ...
            try:
                ...
                token = get_token()
                token_from_keyring = token is not None
            except ImportError:
                pass

        if not token:
            raise ValueError(
                "Enterprise mode requires API token. Please provide token or login via CLI."
            )
        ...
        try:
            async with api_client:
                ...
                # Save token to keyring if it was provided via API (not from keyring)
                if not token_from_keyring:
                    try:
                        from donkit_ragops.enterprise.auth import save_token

                        save_token(token)
                        logger.info("Saved API token to keyring")
                    except Exception as e:
